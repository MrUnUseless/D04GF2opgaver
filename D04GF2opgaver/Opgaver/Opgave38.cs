...
namespace D04GF2opgaver.Opgaver
{
    class Opgave38
    {
        ...
        public static void OpgaveRun()
        {
            ...
            //"do-while" loop repeats until user input is valid ("IsRunning" = false) and moves on when valid.
            do
            {
                ...
                if (sizeask == "s")
                {
                    ...
                    price = small;
                    //Input accepted so stop loop ("IsRunning" = false).
                    IsRunning = false;
                }
            ...
            //"while (true)" loop repeats until user input is valid and moves on (IsRunning = false) when valid.
            do
            {
                //Resets "numberflag"
                numberflag = 1;
                ...
                foreach (char mule in antalstring)
                {
                    ...
                    if (mule < '0' || mule > '9')
                    {
                        //If a character in "antalstring" is not a number then do the following: set variable "numberflag" to 0 and stop check (IsRunning2 = false;) as no further checking is necessary.
                        numberflag = 0;
                        IsRunning2 = false;
                    }
                }
                //If "numberflag" was not set to 0 (not altered as in only number(s) present in user input ("antalstring") then do the following "IsRunning2 = false" stops loop).
                if (numberflag != 0)
                {
                    IsRunning2 = false;
                }
