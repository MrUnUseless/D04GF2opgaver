...
namespace D04GF2opgaver.Opgaver
{
    class Opgave46
    {
        ...
        //Sets svømmebilleter available (using array "string[] SvømmeBillet").
        public static string[] SvømmeBillet = { "Voksen (fra 16 år) ", "Rabatgruppe ", "Børne (7-15 år) ", "Børne (7-15 år) ferie", "Voksen 10-turs", "Rabatgruppe 10-turs", "Børne (7-15 år) 10-turs" };

        //Sets svømmebillet pricing (using array "int[] SvømmeBilletPris").
        public static int[] SvømmeBilletPris = { 42, 23, 15, 10, 330, 175, 135 };

        //Initializes individual svømmebillet type's amount (using array "int[] SvømmeBilletAntal").
        public static int[] SvømmeBilletAntal = new int[7];

        //Initializes individual svømmebillet type's total price (using array "int[] SvømmeBilletPrisTotal").
        public static int[] SvømmeBilletPrisTotal = new int[7];

        //Presets svømmebillet order total price.
        ...
        {            
            ...
            do
            {
                ...
                //Sets "SvømmeBilletPrisTotal[X]" according to price and ordered amount per type of svømmebillet using "TypeTotalPriceCount" as counter to go through the arrays in sync.                
                for (int TypeTotalPriceCount = 0; TypeTotalPriceCount < 7; TypeTotalPriceCount ++)
                {
                SvømmeBilletPrisTotal[TypeTotalPriceCount] = SvømmeBilletPris[TypeTotalPriceCount] * SvømmeBilletAntal[TypeTotalPriceCount];
                }

                //Sets "SamletPrisTotal" to sum value of "SvømmeBilletPrisTotal[X]" by using ".Sum()" to sum all values of array ("SvømmeBilletPrisTotal[0]" + "SvømmeBilletPrisTotal[1]" + ....+ "SvømmeBilletPrisTotal[7]").
                SamletPrisTotal = SvømmeBilletPrisTotal.Sum();

                //Menu display part.

                //Clears console and presents menu using the "\n for spacing and the "$" combined with "{}" to call the various svømmebillet related variables.
                Console.Clear();
                Console.SetCursorPosition(ScreenCenterX - 20, 1);
                Console.WriteLine("Velkommen til Ballerup Svømmehal!");                
                Console.WriteLine("\nHerunder vises vores billet typer. Børn under 7 år kommer gratis ind i følge med en voksen.");
                Console.WriteLine("\nPensionister, efterlønsmodtagere, arbejdsledige, borgere på ledighedsydelse,");
                Console.WriteLine("dagpengemodtagere og studerende er i rabatgruppen.");
                Console.ForegroundColor = ConsoleColor.Red;
                Console.WriteLine("Vær opmærksom på at dokumentation er påkrævet for rabatgruppen!");
                Console.ForegroundColor = ConsoleColor.White;
                Console.WriteLine("\nI skoleferierne (sommer/efterår/vinter) kan børn købe feriebilletter");
                ...
                for (int MenuPrint = 0; MenuPrint < 7; MenuPrint ++)
                        {                            
                            Console.WriteLine($"\n {MenuPrint + 1}: {SvømmeBillet[MenuPrint]}billet til {SvømmeBilletPris[MenuPrint]} kr.");                            
                        }
                ...
                if (SamletPrisTotal != 0)
                {
                    ...
                    //Uses "TypeOrderCount" as counter to go through array "SvømmeBilletAntal" to check if it has value different from 0 and if so display order in counter sync with the 2 other arrays.
                    for (int TypeOrderCount = 0; TypeOrderCount < 7; TypeOrderCount ++)
                        {
                            if (SvømmeBilletAntal[TypeOrderCount] != 0)
                            {
                            Console.WriteLine($" {SvømmeBilletAntal[TypeOrderCount]} {SvømmeBillet[TypeOrderCount]}billet(er) af ialt {SvømmeBilletPrisTotal[TypeOrderCount]} kr.");
                            }
                        }                    
                ...
                /*Sets variable "tast" as user input and runs "switch (tast)" accordingly as in adding to amount of svømmebilleter or ending order.
                If input ("Tast") is 1-7 then "TastInputCaseResolver" is called to start resolving current input.
                That in turn calls "IntInputValidCheck" to determine a valid amount of svømmebillet chosen before returning that to "TastInputCaseResolver".
                ...
                switch (Tast)
                {
                    ...
                    default:
                        ...
                        Console.WriteLine("\nDu har ikke tastet 1, 2, 3, 4, 5, 6, 7 eller 9.\nTryk en tast for at prøve igen:");
                        Console.ReadKey();
                        break;
                }
        ...
        public static void TastInputCaseResolver()
        {
            ...
            IIValidCheckCallerPrint = ($"\nHvor mange {SvømmeBillet[IntTast - 1]}billet(er) ønsker du at købe?\n");
            IntInputValidCheck();
            ...
            Console.WriteLine($"\nDu har føjet {ItemAmountResult} {SvømmeBillet[IntTast - 1]}billet(er) til din bestilling\nTryk en tast for at vende tilbage til menuen:");
            SvømmeBilletAntal[IntTast - 1] = SvømmeBilletAntal[IntTast - 1] + ItemAmountResult;
            Console.ReadKey();
